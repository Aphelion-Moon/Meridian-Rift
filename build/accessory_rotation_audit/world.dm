...
/world/New()
	...
	for(var/datum/sprite_accessory/path as anything in subtypesof(/datum/sprite_accessory))
		...
		rows += list(list("type" = "[path]", "name" = initial(path.name), "key" = initial(path.key), "icon" = "[icon_file]", "state" = initial(path.icon_state), "center" = initial(path.center), "x" = initial(path.dimension_x), "y" = initial(path.dimension_y), "width" = canvas.Width(), "height" = canvas.Height(), "special_x" = initial(path.special_x_dimension)))
	text2file(json_encode(rows), "build/accessory_rotation_audit/canvases.json")
	var/datum/unit_test/accessory_icon_canvases/audit/test = new
	test.Run()
	text2file(json_encode(list("passed" = test.succeeded, "failures" = test.fail_reasons)), "build/accessory_rotation_audit/test_result.json")
	shutdown()
	return
...
/datum/unit_test/accessory_icon_canvases/audit/New()
	return
