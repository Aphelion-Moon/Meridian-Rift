...
/atom/proc/process_exposure()
	...
	if(!isopenturf(loc))
		...
		flags_1 &= ~ATMOS_IS_PROCESSING_1
		return
	if(!should_atmos_process(spot.air, spot.air.return_temperature())) //Things can change without a tile becoming active // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!should_atmos_process(spot.air, spot.air.temperature)) //Things can change without a tile becoming active
		atmos_end()
		SSair.atom_process -= src
		flags_1 &= ~ATMOS_IS_PROCESSING_1
		return
	atmos_expose(spot.air, spot.air.return_temperature()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: atmos_expose(spot.air, spot.air.temperature)

/turf/open/process_exposure()
	if(!should_atmos_process(air, air.return_temperature())) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!should_atmos_process(air, air.temperature))
		atmos_end()
		SSair.atom_process -= src
		flags_1 &= ~ATMOS_IS_PROCESSING_1
		return
	atmos_expose(air, air.return_temperature()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: atmos_expose(air, air.temperature)
