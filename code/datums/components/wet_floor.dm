...
/datum/component/wet_floor/process()
	...
	switch(t)
		...
		if(T0C to T0C + 100)
			decrease = ((T.air.return_temperature() - T0C) / SSwet_floors.temperature_coeff) * (diff / SSwet_floors.time_ratio) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: decrease = ((T.air.temperature - T0C) / SSwet_floors.temperature_coeff) * (diff / SSwet_floors.time_ratio)
