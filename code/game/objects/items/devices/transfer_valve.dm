...
/obj/item/transfer_valve/proc/merge_gases(obj/item/tank/target, change_volume = TRUE)
	...
	if(change_volume)
		target_mix.set_volume(target_mix.return_volume() + other_mix.return_volume()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: target_mix.volume += other_mix.volume
...
/obj/item/transfer_valve/proc/split_gases()
	...
	var/volume_ratio = mix_one.return_volume()/mix_two.return_volume() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/volume_ratio = mix_one.volume/mix_two.volume
	var/datum/gas_mixture/temp
	temp = mix_two.remove_ratio(volume_ratio)
	mix_one.merge(temp)
	mix_two.set_volume(mix_two.return_volume() - mix_one.return_volume()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: mix_two.volume -= mix_one.volume
