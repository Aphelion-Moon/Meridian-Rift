...
/obj/structure/closet/crate/freezer/process_internal_air(seconds_per_tick)
	if(opened)
		...
		internal_air.set_temperature(max(current_exposed_air.return_temperature(), internal_air.return_temperature())) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: internal_air.temperature = max(current_exposed_air.temperature, internal_air.temperature)
		return ..()
	else
		if(internal_air.return_temperature() <= minimum_temperature) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(internal_air.temperature <= minimum_temperature)
			return
