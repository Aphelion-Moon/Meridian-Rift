...
/obj/machinery/space_heater/process_atmos()
	...
	if(set_mode != HEATER_MODE_COOL && enviroment.return_temperature() < target_temperature - temperature_tolerance) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(set_mode != HEATER_MODE_COOL && enviroment.temperature < target_temperature - temperature_tolerance)
		new_mode = HEATER_MODE_HEAT
	else if(set_mode != HEATER_MODE_HEAT && enviroment.return_temperature() > target_temperature + temperature_tolerance) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: else if(set_mode != HEATER_MODE_HEAT && enviroment.temperature > target_temperature + temperature_tolerance)
	...
	var/required_energy = abs(enviroment.return_temperature() - target_temperature) * enviroment.heat_capacity() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/required_energy = abs(enviroment.temperature - target_temperature) * enviroment.heat_capacity()
	...
	for(var/turf/open/turf in turfs)
		...
		turf_gasmix.set_temperature(turf_gasmix.return_temperature() + delta_energy / turf_gasmix.heat_capacity()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: turf_gasmix.temperature += delta_energy / turf_gasmix.heat_capacity()
...
/obj/machinery/space_heater/ui_data()
	...
	if(istype(local_turf))
		...
		*/ // APHELION EDIT REMOVAL END
		current_temperature = local_turf.GetTemperature() // APHELION EDIT ADDITION - DOGMOS
