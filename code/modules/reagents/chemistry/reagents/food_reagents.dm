...
/datum/reagent/consumable/nutriment/fat/expose_turf(turf/open/exposed_turf, reac_volume)
	...
	if(hotspot)
		...
		lowertemp.set_temperature(max( min(lowertemp.return_temperature()-2000,lowertemp.return_temperature() / 2) ,0)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: lowertemp.temperature = max( min(lowertemp.temperature-2000,lowertemp.temperature / 2) ,0)
		lowertemp.react(src)
...
/datum/reagent/consumable/frostoil/expose_turf(turf/exposed_turf, reac_volume)
	...
	if(isopenturf(exposed_turf))
		...
		var/temperature = exposed_open_turf.air.return_temperature() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/temperature = exposed_open_turf.air.temperature
		var/heat_capacity = exposed_open_turf.air.heat_capacity()
		exposed_open_turf.air.set_temperature(max(exposed_open_turf.air.return_temperature() - ((temperature - TCMB) * (heat_capacity * reac_volume * specific_heat) / (heat_capacity + reac_volume * specific_heat)) / heat_capacity, TCMB)) // Exchanges environment temperature with reagent. Reagent is at 2.7K with a heat capacity of 40J per unit. // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: exposed_open_turf.air.temperature = max(exposed_open_turf.air.temperature - ((temperature - TCMB) * (heat_capacity * reac_volume * specific_heat) / (heat_capacity + reac_volume * specific_heat)) / heat_capacity, TCMB) // Exchanges environment temperature with reagent. Reagent is at 2.7K with a heat capacity of 40J per unit.
	if(reac_volume < 5)
		return
