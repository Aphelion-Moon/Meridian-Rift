...
// APHELION EDIT ADDITION START - HEMOPHAGE_IMPORT
/// Legacy hemophage slots become humanoid with the hemophage quirk in player and staff imports.
/datum/unit_test/preferences_import_hemophage_migration/Run()
	var/hemophage_quirk = /datum/quirk/hemophage::name
	// Player imports supply local metadata; existing quirks must survive the migration.
	var/list/slot = list("species" = "hemophage", "all_quirks" = list(/datum/quirk/clumsy::name))
	prefs_import_pass1(list("version" = 52, "character1" = slot), list())
	TEST_ASSERT_EQUAL(slot["species"], SPECIES_HUMANOID, "A player import did not convert legacy hemophage species to humanoid.")
	TEST_ASSERT(hemophage_quirk in slot["all_quirks"], "A player import did not grant the hemophage quirk.")
	TEST_ASSERT(/datum/quirk/clumsy::name in slot["all_quirks"], "A player import dropped existing quirks while migrating hemophage species.")

	// Staff imports omit local metadata and restore the complete backup; absent quirks must still migrate.
	slot = list("species" = "hemophage")
	prefs_import_pass1(list("version" = 52, "character1" = slot))
	TEST_ASSERT_EQUAL(slot["species"], SPECIES_HUMANOID, "A staff import did not convert legacy hemophage species to humanoid.")
	TEST_ASSERT(hemophage_quirk in slot["all_quirks"], "A staff import did not grant the hemophage quirk when quirks were absent.")

	// A malformed quirk entry must not block the migration.
	slot = list("species" = "hemophage", "all_quirks" = 999)
	prefs_import_pass1(list("version" = 52, "character1" = slot))
	TEST_ASSERT_EQUAL(slot["species"], SPECIES_HUMANOID, "A malformed quirk list blocked hemophage species migration.")
	TEST_ASSERT(hemophage_quirk in slot["all_quirks"], "A malformed quirk list blocked the hemophage quirk.")

	// An existing hemophage quirk must not duplicate.
	slot = list("species" = "hemophage", "all_quirks" = list(hemophage_quirk, /datum/quirk/clumsy::name))
	prefs_import_pass1(list("version" = 52, "character1" = slot))
	var/hemophage_count = 0
	for(var/quirk in slot["all_quirks"])
		if(quirk == hemophage_quirk)
			hemophage_count++
	TEST_ASSERT_EQUAL(hemophage_count, 1, "Migrating a slot that already had the hemophage quirk duplicated it.")
	TEST_ASSERT(/datum/quirk/clumsy::name in slot["all_quirks"], "Deduplicating the hemophage quirk dropped existing quirks.")

	// Other species must pass through without gaining the quirk.
	slot = list("species" = SPECIES_LIZARD, "all_quirks" = list(/datum/quirk/clumsy::name))
	prefs_import_pass1(list("version" = 52, "character1" = slot))
	TEST_ASSERT_EQUAL(slot["species"], SPECIES_LIZARD, "Migration changed a non-hemophage species.")
	TEST_ASSERT(!(hemophage_quirk in slot["all_quirks"]), "Migration granted the hemophage quirk to a non-hemophage species.")
	TEST_ASSERT_EQUAL(length(slot["all_quirks"]), 1, "Migration changed quirks for a non-hemophage species.")

	// A repeat pass must remain stable.
	slot = list("species" = "hemophage", "all_quirks" = list())
	prefs_import_pass1(list("version" = 52, "character1" = slot))
	prefs_import_pass1(list("version" = 52, "character1" = slot))
	TEST_ASSERT_EQUAL(slot["species"], SPECIES_HUMANOID, "A second pass changed the migrated species.")
	hemophage_count = 0
	for(var/repeated_quirk in slot["all_quirks"])
		if(repeated_quirk == hemophage_quirk)
			hemophage_count++
	TEST_ASSERT_EQUAL(hemophage_count, 1, "A second import pass duplicated the hemophage quirk.")

	// A foreign modular version must not block the migration.
	slot = list("species" = "hemophage", "modular_version" = 999, "all_quirks" = list())
	prefs_import_pass1(list("version" = 52, "character1" = slot))
	TEST_ASSERT_EQUAL(slot["species"], SPECIES_HUMANOID, "A foreign modular version blocked hemophage species migration.")
	TEST_ASSERT(hemophage_quirk in slot["all_quirks"], "A foreign modular version blocked the hemophage quirk.")
