...
/datum/unit_test/preferences_pda_theme_migration/Run()
	...
	for(var/legacy_name in legacy_names)
		...
		TEST_ASSERT_NOTEQUAL(legacy_name, canonical_name, "Legacy name '[legacy_name]' is its own migration target, so translating it is dead code.")
		TEST_ASSERT(!(legacy_name in GLOB.default_pda_themes), "Legacy name '[legacy_name]' leaked into PDA preference choices.")
		TEST_ASSERT(!(legacy_name in GLOB.pda_name_to_theme), "Legacy name '[legacy_name]' leaked into the clear PDA theme catalog.")
	...
	TEST_ASSERT_EQUAL(preference.deserialize(list(PDA_THEME_NTOS_LEGACY_NAME), null), preference.create_default_value(), "A non-text PDA theme did not fall back to the default.")
...
/datum/unit_test/preferences_meridian_theme/Run()
	...
	TEST_ASSERT_EQUAL(preference.create_default_value(), "meridian_aphelion", "MeridianOS Aphelion must remain the default.")
	...
	TEST_ASSERT(!preference.is_valid(list("meridian_electra"), null), "MeridianOS theme accepted a non-text raw value.")
	...
	test_preferences.value_cache[preference.type] = "meridian_electra"
	...
	other_preferences.value_cache[preference.type] = "meridian_electra"
	...
	TEST_ASSERT_EQUAL(test_preferences.read_preference(preference.type), "meridian_electra", "Rejected input changed the saved theme.")
	...
	TEST_ASSERT(test_client.set_meridian_theme("meridian_wastelander"), "A valid theme selection was rejected.")
	TEST_ASSERT_EQUAL(test_preferences.read_preference(preference.type), "meridian_wastelander", "A valid theme selection was not applied.")
	...
	TEST_ASSERT_EQUAL(test_lobby.captured_theme, "meridian_wastelander", "The lobby received a non-canonical preference value.")

	TEST_ASSERT(test_client.set_meridian_theme("meridian_wastelander"), "Reselecting the canonical value should be a successful no-op.")
	...
	TEST_ASSERT(test_client.set_meridian_theme("meridian_hotline"), "The Hotline theme selection was rejected.")
	TEST_ASSERT_EQUAL(test_preferences.read_preference(preference.type), "meridian_hotline", "The Hotline selection was not applied.")
	TEST_ASSERT_EQUAL(preference.deserialize(preference.serialize("meridian_hotline"), null), "meridian_hotline", "The saved Hotline selection did not round-trip.")
	TEST_ASSERT_EQUAL(test_preferences.save_call_count, 3, "The Hotline selection was not persisted exactly once.")
	TEST_ASSERT_EQUAL(first_ui.config_update_count, 3, "The first open TGUI did not receive the Hotline update.")
	TEST_ASSERT_EQUAL(second_ui.config_update_count, 3, "The second open TGUI did not receive the Hotline update.")
	TEST_ASSERT_EQUAL(test_lobby.update_count, 3, "The open lobby did not receive the Hotline update.")
	TEST_ASSERT_EQUAL(test_lobby.captured_theme, "meridian_hotline", "The lobby received the wrong Hotline preference ID.")

	TEST_ASSERT_EQUAL(other_preferences.read_preference(preference.type), "meridian_electra", "One client's selection mutated another preference owner.")
