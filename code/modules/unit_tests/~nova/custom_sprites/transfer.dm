...
/datum/unit_test/custom_style_taur_transfer/Run()
	...
	for(var/zone in list(null, "taur"))
		...
		var/text = custom_style_export_text(package)
		var/list/result = custom_style_parse(text)
		TEST_ASSERT(!(result["error"] || result["package"]?["drawing"]?["version"] != 3), "Wide whole-body and taur exports must round-trip: [result["error"]]")
		var/list/exported = json_decode(text)
	...
	TEST_ASSERT(!(custom_style_parse(custom_style_test_export("markings", null))["error"] || custom_style_parse(custom_style_export_text(custom_style_package("markings", "taur", null, null)))["error"]), "Legacy whole-body art and explicit empty taur-zone art must remain valid.")
