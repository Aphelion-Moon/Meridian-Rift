...
/datum/unit_test/custom_style_previous_rotation/Run()
	...
	TEST_ASSERT(!(custom_sprite_hash(preferences.custom_limb_markings[BODY_ZONE_R_ARM]) != custom_sprite_hash(second) || preferences.custom_style_previous_package("markings", BODY_ZONE_R_ARM) || preferences.custom_markings), "Rotating one zone must not affect other zones or whole-body markings.")
