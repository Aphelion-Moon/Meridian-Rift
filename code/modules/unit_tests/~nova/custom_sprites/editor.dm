...
/datum/unit_test/custom_sprite_editor_lifecycle/Run()
	...
	for(var/target in list("hair", "markings"))
		...
		TEST_ASSERT(!(!saved || preferences.custom_sprite_editors?[target]), "Closing must save the drawing and release its editor.")
...
/datum/unit_test/custom_sprite_explicit_tint_reopen/Run()
	...
	for(var/target in list("hair", "markings"))
		...
		TEST_ASSERT(!(literal?["tint"] != "#ffffff" || !("#80c040" in literal["palette"]) || custom_sprite_hash(target == "hair" ? preferences.custom_hair : preferences.custom_markings) != original_hash), "Opening an explicitly tinted drawing must bake exact RGB without saving or aliasing its stored data.")
		var/mob/living/carbon/human/body = editor.preview_body
...
/datum/unit_test/custom_sprite_emissive_editor/Run()
	...
	for(var/target in list("hair", "markings"))
		...
		var/list/bounds = editor.workspace.draw_bounds["2"]
		TEST_ASSERT(editor.workspace.new_transaction(list("type" = "pencil", "layer" = 1, "dir" = "2", "color" = "[editor.workspace.palette[1]]ff", "points" = list(list(bounds[1], bounds[2])))), "The emissive persistence fixture must paint a valid pixel.")
		...
		editor.finish(FALSE)
	TEST_ASSERT(!(preferences.custom_hair["emissive"]["2"] || !preferences.custom_markings["emissive"]["2"] || !preferences.read_preference(/datum/preference/toggle/hair_emissive)), "Hair and markings must retain independent choices without changing the hair preference.")
	...
	TEST_ASSERT(!(slot_data?["hair"]?["emissive"]?["2"] || !slot_data?["markings"]?["emissive"]?["2"] || !slot_data?["hair"]?["emissive"]?["1"] || slot_data?["markings"]?["emissive"]?["1"]), "Both layers' independent emissive settings must survive a disk reload.")
	...
	TEST_ASSERT(!(blocked_data["emissiveAllowed"] || blocked.ui_act("setEmissive", list("dir" = "2", "enabled" = TRUE), blocked_ui, null) || json_encode(blocked.preview_body.dna.custom_markings["emissive"]) != json_encode(custom_sprite_emissive_settings(FALSE))), "The character's master emissive preference must gate both the editor and rendered drawing.")
	blocked.finish(FALSE)
	TEST_ASSERT(preferences.custom_markings["emissive"]["2"], "The master preference must suppress appearance without overwriting the saved layer setting.")
	...
	TEST_ASSERT(!(json_encode(body.dna.custom_hair["emissive"]) != json_encode(custom_sprite_emissive_settings(FALSE)) || json_encode(body.dna.custom_markings["emissive"]) != json_encode(custom_sprite_emissive_settings(FALSE))), "Ordinary preference application must respect the master emissive switch.")
	preferences.write_preference(GLOB.preference_entries[/datum/preference/toggle/allow_emissives], TRUE)
	preferences.apply_prefs_to(body, TRUE, visuals_only = TRUE)
	TEST_ASSERT(!(body.dna.custom_hair["emissive"]["2"] || !body.dna.custom_markings["emissive"]["2"] || !body.dna.custom_hair["emissive"]["1"] || body.dna.custom_markings["emissive"]["1"]), "Re-enabling emissives must restore the saved independent layer settings.")
	...
	for(var/hair_emissive in list(FALSE, TRUE))
		...
		preferences.apply_prefs_to(body, TRUE, visuals_only = TRUE)
		TEST_ASSERT(!(json_encode(body.dna.custom_hair["emissive"]) != json_encode(preferences.custom_hair["emissive"]) || json_encode(body.dna.custom_markings["emissive"]) != json_encode(preferences.custom_markings["emissive"]) || custom_sprite_hash(preferences.custom_hair) != hair_hash || custom_sprite_hash(preferences.custom_markings) != markings_hash), "Normal hair emission changes must not alter either custom drawing's saved or applied flags.")
...
/datum/unit_test/custom_sprite_reopen_clear_render/Run()
	...
	for(var/target in list("hair", "markings"))
		...
		for(var/tool in list("eraser", "clear"))
			...
			editor.finish(TRUE)
			TEST_ASSERT((target == "hair" ? preferences.custom_hair : preferences.custom_markings), "Fixture must save its drawing before reopening.")
			editor = new /datum/custom_sprite_editor/optimization_test(preferences, target)
			LAZYSET(preferences.custom_sprite_editors, target, editor)
			...
			editor.finish(TRUE)
			TEST_ASSERT(!(target == "hair" ? preferences.custom_hair : preferences.custom_markings), "Saving cleared [target] must persist its removal.")
			TEST_ASSERT(custom_sprite_hash(other_target == "hair" ? preferences.custom_hair : preferences.custom_markings) == other_hash, "Editing [target] must preserve the other target's independent drawing.")
...
/datum/unit_test/custom_marking_zone_editor/Run()
	...
	for(var/body_zone in GLOB.custom_marking_zone_labels)
		...
		editor.refresh_preview()
		TEST_ASSERT(custom_sprite_hash(editor.preview_body.dna.custom_markings) == custom_sprite_hash(custom_sprite_appearance_drawing(preferences.custom_markings, preferences.read_preference(/datum/preference/toggle/allow_emissives))), "Zone previews must retain the whole-body marking.")
		...
		editor.finish(TRUE)
		TEST_ASSERT(!(!preferences.custom_limb_markings?[body_zone] || custom_sprite_hash(preferences.custom_markings) != whole_hash || preferences.custom_sprite_editors?["markings:[body_zone]"]), "Saving a zone must preserve whole-body paint and release only its own editor.")
		...
		editor.finish(TRUE)
		TEST_ASSERT(!(preferences.custom_limb_markings?[body_zone] || custom_sprite_hash(preferences.custom_markings) != whole_hash), "Clearing a reopened zone must preserve the whole-body marking.")
...
/datum/unit_test/custom_sprite_taur_canvas/Run()
	...
	for(var/zone in list(null, "taur"))
		...
		TEST_ASSERT(!(editor.workspace.width != 64 || editor.workspace.height != 32), "Whole-body and taur-only drawings need the native 64 by 32 taur canvas.")
		for(var/direction in GLOB.cardinals)
			var/icon/guide = editor.guide_icons["[direction]"]
...
/datum/unit_test/custom_sprite_taur_import_preview/Run()
	...
	TEST_ASSERT(editor.show_candidate(custom_style_package("markings", null, drawing, null), "import"), "A centered legacy marking inside the taur silhouette must be importable.")
	var/list/proposed = editor.candidate["previews"]
	TEST_ASSERT(editor.apply_candidate(), "The reviewed legacy marking must replace the draft.")
	editor.refresh_preview()
...
/datum/unit_test/custom_sprite_base_markings/Run()
	...
	TEST_ASSERT(!(!editor.can_change_markings() || whole_body.can_change_markings()), "Only a limb zone may change its own markings.")
