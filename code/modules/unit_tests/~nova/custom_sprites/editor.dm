...
/datum/unit_test/custom_sprite_editor_backgrounds/Run()
	var/datum/client_interface/mock_client = allocate(/datum/client_interface)
	var/datum/preferences/preferences = allocate(/datum/preferences/preferences_import_test, mock_client)
	preferences.write_preference(GLOB.preference_entries[/datum/preference/choiced/background_state], "Plating")
	var/datum/custom_sprite_editor/editor = new /datum/custom_sprite_editor/optimization_test(preferences, "hair")
	var/list/data = editor.ui_static_data(mock_client.mob)
	TEST_ASSERT(data["defaultBackground"] == "Plating", "The swatches must open on the character's own preview background.")
	TEST_ASSERT(length(data["backgrounds"]) == length(GLOB.background_state_options), "Every preview background must be offered.")
	for(var/list/tile as anything in data["backgrounds"])
		TEST_ASSERT(!(findtext(tile["url"], "data:image/png;base64,") != 1 || findtext(tile["wideUrl"], "data:image/png;base64,") != 1), "Tiles must be self-contained PNG data URLs.")
	TEST_ASSERT(preferences.read_preference(/datum/preference/choiced/background_state) == "Plating", "Opening the editor must not touch the preference.")
	editor.finish(FALSE)

/datum/unit_test/custom_style_rewrite_markings/Run()
	var/list/entries = list(list("name" = "A", "color" = "#111111", "emissive" = FALSE), list("name" = "B", "color" = "#222222", "emissive" = TRUE))
	var/list/renamed = custom_style_rewrite_markings(entries, 1, "C", null)
	TEST_ASSERT(!(renamed[1]["name"] != "C" || renamed[1]["color"] != "#111111" || renamed[2]["name"] != "B"), "Swapping a marking keeps its color and place.")
	var/list/added = custom_style_rewrite_markings(entries, null, "D", "#333333")
	TEST_ASSERT(!(length(added) != 3 || added[3]["color"] != "#333333"), "Adding appends with the given color.")
	TEST_ASSERT(length(custom_style_rewrite_markings(entries, 2, null, null)) == 1, "Removing drops only that marking.")
	TEST_ASSERT(isnull(custom_style_rewrite_markings(entries, 5, null, null)), "Acting on a missing marking fails.")

/datum/unit_test/custom_sprite_editor_body_import/Run()
	var/datum/client_interface/mock_client = allocate(/datum/client_interface)
	var/datum/preferences/preferences = allocate(/datum/preferences/preferences_import_test, mock_client)
	preferences.write_preference(GLOB.preference_entries[/datum/preference/choiced/species], SPECIES_HUMAN)
	var/datum/custom_sprite_editor/zone_editor = new /datum/custom_sprite_editor/qualification(preferences, "markings", BODY_ZONE_L_ARM)
	var/list/point = custom_sprite_test_paintable_point(zone_editor)
	...
	var/list/result = custom_style_parse(custom_style_body_export_text(list(BODY_ZONE_L_ARM = custom_style_package("markings", BODY_ZONE_L_ARM, custom_sprite_validate(arm), null))))
	TEST_ASSERT(result["body"], "The fixture must parse as a whole-body style: [result["error"]]")
	TEST_ASSERT(zone_editor.preview_received(result), "A single-zone markings editor must preview its own region from a whole-body file: [zone_editor.transfer_error]")
	var/datum/custom_sprite_editor/leg_editor = new /datum/custom_sprite_editor/qualification(preferences, "markings", BODY_ZONE_R_LEG)
	TEST_ASSERT(!(leg_editor.preview_received(result) || !findtext(leg_editor.transfer_error, "right leg")), "A whole-body file without this zone must be refused, naming the zone.")
	var/datum/custom_sprite_editor/hair_editor = new /datum/custom_sprite_editor/qualification(preferences, "hair", null)
	TEST_ASSERT(!(hair_editor.preview_received(result) || !findtext(hair_editor.transfer_error, "whole-body")), "A hair editor must refuse a whole-body file and say why.")
	for(var/datum/custom_sprite_editor/editor as anything in list(zone_editor, leg_editor, hair_editor))
		editor.finish(FALSE)

/// A stroke in a hair window shows in its preview once the debounce runs, as a markings stroke does.
/datum/unit_test/custom_sprite_hair_preview_follows_strokes/Run()
	var/datum/client_interface/mock_client = allocate(/datum/client_interface)
	var/datum/preferences/preferences = allocate(/datum/preferences/preferences_import_test, mock_client)
	preferences.write_preference(GLOB.preference_entries[/datum/preference/choiced/species], SPECIES_HUMAN)
	for(var/target in GLOB.custom_style_hair_targets)
		...
		TEST_ASSERT(editor.preview_timer, "A [target] stroke must schedule a preview refresh.")
...
/datum/unit_test/custom_sprite_cover_static_data/Run()
	var/datum/client_interface/mock_client = allocate(/datum/client_interface)
	var/datum/preferences/preferences = allocate(/datum/preferences/preferences_import_test, mock_client)
	var/datum/custom_sprite_editor/editor = new /datum/custom_sprite_editor/qualification(preferences, "markings", BODY_ZONE_CHEST)
	LAZYSET(preferences.custom_sprite_editors, editor.editor_key, editor)
	var/list/cover = editor.ui_static_data(mock_client.mob)["coverMask"]
	TEST_ASSERT(length(cover) == 1 && length(cover["2"]) == 32, "Opening must publish the Front view's cover rows with its guide.")
	editor.render_view("1")
	cover = editor.ui_static_data(mock_client.mob)["coverMask"]
	TEST_ASSERT(length(cover["1"]) == 32, "Showing a view must publish its cover rows.")
	var/datum/custom_sprite_editor/hair = new /datum/custom_sprite_editor/qualification(preferences, "hair", null)
...
/// A view whose drawable box is empty has no bounds; syncing locks must leave it alone rather than copy null.
/datum/unit_test/custom_sprite_locked_views_without_bounds/Run()
	var/datum/client_interface/mock_client = allocate(/datum/client_interface)
	var/datum/preferences/preferences = allocate(/datum/preferences/preferences_import_test, mock_client)
	var/datum/custom_sprite_editor/editor = new /datum/custom_sprite_editor/optimization_test(preferences, "markings", BODY_ZONE_CHEST)
	LAZYSET(preferences.custom_sprite_editors, editor.editor_key, editor)
	editor.unlocked_bounds["1"] = null
	TEST_ASSERT(editor.sync_locked_views(push = FALSE), "Losing a view's box must count as a change.")
	TEST_ASSERT(isnull(editor.workspace.draw_bounds["1"]), "A view without a box must lose its drawing bounds.")
	TEST_ASSERT(!editor.sync_locked_views(push = FALSE), "A view without a box must stay unchanged on the next sync.")
	editor.finish(FALSE)

/datum/custom_sprite_editor/optimization_test/locked_back/locked_directions()
	return list("1")

/// A locked view still publishes its real cover rows, so unlocking it later hatches paint at once.
/datum/unit_test/custom_sprite_cover_rows_ignore_view_locks/Run()
	var/datum/client_interface/mock_client = allocate(/datum/client_interface)
	var/datum/preferences/preferences = allocate(/datum/preferences/preferences_import_test, mock_client)
	preferences.write_preference(GLOB.preference_entries[/datum/preference/choiced/hairstyle], "Business Hair")
	var/datum/custom_sprite_editor/editor = new /datum/custom_sprite_editor/optimization_test/locked_back(preferences, "markings", BODY_ZONE_CHEST)
	LAZYSET(preferences.custom_sprite_editors, editor.editor_key, editor)
	TEST_ASSERT(editor.workspace.draw_bounds["1"] ~= list(0, 0, -1, -1), "The fixture's Back view must be locked.")
	editor.render_view("1")
	TEST_ASSERT(findtext(jointext(editor.cover_rows["1"], ""), "1"), "A locked view's cover rows must still mark the hair.")
	editor.finish(FALSE)
