...
/datum/unit_test/dogmos_kennel_overlays/Run()
	...
	SSair.diagnostics.kennel_overlay_breach_turfs = list()
	marked_turfs = list()
	...
	for(var/i in 1 to KENNEL_OVERLAY_RECENT_CAP)
		...
		SSair.diagnostics.kennel_mark_overlay_recent(SSair.diagnostics.kennel_overlay_breach_turfs, KENNEL_OVERLAY_BREACH, T)
	...
	TEST_ASSERT_EQUAL(length(SSair.diagnostics.kennel_overlay_breach_turfs), KENNEL_OVERLAY_RECENT_CAP, \
		"After marking exactly KENNEL_OVERLAY_RECENT_CAP ([KENNEL_OVERLAY_RECENT_CAP]) distinct turfs, the tracker holds [length(SSair.diagnostics.kennel_overlay_breach_turfs)] - none should have been evicted yet.")
	...
	SSair.diagnostics.kennel_mark_overlay_recent(SSair.diagnostics.kennel_overlay_breach_turfs, KENNEL_OVERLAY_BREACH, overflow_turf)
	marked_turfs += overflow_turf

	TEST_ASSERT_EQUAL(length(SSair.diagnostics.kennel_overlay_breach_turfs), KENNEL_OVERLAY_RECENT_CAP, \
		"After exceeding the cap by one, the tracker holds [length(SSair.diagnostics.kennel_overlay_breach_turfs)], expected it to stay at KENNEL_OVERLAY_RECENT_CAP ([KENNEL_OVERLAY_RECENT_CAP]) via eviction.")
	TEST_ASSERT(!first_marked.vis_contents.Find(first_slots[first_offset]), \
	...
	TEST_ASSERT(!(first_marked in SSair.diagnostics.kennel_overlay_breach_turfs), \
		"The oldest turf is still present in the tracker list after eviction.")
	TEST_ASSERT((overflow_turf in SSair.diagnostics.kennel_overlay_breach_turfs), \
	...
	SSair.diagnostics.kennel_mark_overlay_recent(SSair.diagnostics.kennel_overlay_breach_turfs, KENNEL_OVERLAY_BREACH, second_marked)
	TEST_ASSERT_EQUAL(length(SSair.diagnostics.kennel_overlay_breach_turfs), KENNEL_OVERLAY_RECENT_CAP, \
		"Re-marking an already-tracked turf changed the tracker size to [length(SSair.diagnostics.kennel_overlay_breach_turfs)] - it should be moved to the front, not duplicated.")
