 ...
 * remove_from_group() - a group's peak_size is recorded into SSair.diagnostics.recent_fire_groups exactly once,
...
/datum/unit_test/dogmos_kennel_fire_groups/Run()
	...
	SSair.diagnostics.recent_fire_groups = list()
	...
	SSair.diagnostics.kennel_fire_group_notable_size = 2
	...
	TEST_ASSERT_EQUAL(length(SSair.diagnostics.recent_fire_groups), 0, \
	...
	TEST_ASSERT_EQUAL(length(SSair.diagnostics.recent_fire_groups), 1, \
		"A fire group with peak_size 2, at kennel_fire_group_notable_size (2), was not recorded - expected 1 entry, got [length(SSair.diagnostics.recent_fire_groups)].")
	TEST_ASSERT_EQUAL(SSair.diagnostics.recent_fire_groups[1]["peak_size"], 2, \
		"The recorded entry's peak_size ([SSair.diagnostics.recent_fire_groups[1]["peak_size"]]) does not match the group's real peak (2).")
