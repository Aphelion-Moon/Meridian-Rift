...
#define GET_MOLES(gas_mixture, gas_type) (gas_mixture.get_moles(gas_type)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: #define GET_MOLES(gas_mixture, gas_type) (gas_mixture.moles[gas_type] || 0)
...
/datum/unit_test/lungs/proc/create_gas_mix(list/gas_to_percent)
	...
	test_mix.set_temperature(T20C) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: test_mix.temperature = T20C
	for(var/datum/gas/gas_type as anything in gas_to_percent)
		...
		*/ // APHELION EDIT REMOVAL END
		test_mix.set_moles(gas_type, (ONE_ATMOSPHERE * 2500 / (R_IDEAL_GAS_EQUATION * T20C) * gas_to_percent[gas_type])) // APHELION EDIT ADDITION - DOGMOS
	return test_mix
