...
/datum/unit_test/explosion_action/proc/execute_mob_tests()
	...
	EX_ACT(test_dog, EXPLODE_HEAVY) // should do 30 damage.
	TEST_ASSERT_EQUAL(test_dog.health, MAX_LIVING_HEALTH - 30, "EX_ACT() with EXPLODE_HEAVY severity should have done 30 damage to a corgi with an immune helmet and vest!")
	test_dog.revive(ADMIN_HEAL_ALL)
