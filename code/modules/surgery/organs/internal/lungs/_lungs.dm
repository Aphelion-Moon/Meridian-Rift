...
/obj/item/organ/lungs/proc/breathe_oxygen(mob/living/carbon/breather, datum/gas_mixture/breath, o2_pp, old_o2_pp)
	if(o2_pp < safe_oxygen_min && !HAS_TRAIT(breather, TRAIT_NO_BREATHLESS_DAMAGE))
		...
		var/gas_breathed = handle_suffocation(breather, o2_pp, safe_oxygen_min, breath.get_moles(/datum/gas/oxygen)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/gas_breathed = handle_suffocation(breather, o2_pp, safe_oxygen_min, breath.moles[/datum/gas/oxygen])
		if(o2_pp)
...
/obj/item/organ/lungs/proc/too_much_oxygen(mob/living/carbon/breather, datum/gas_mixture/breath, o2_pp, old_o2_pp)
	...
	var/ratio = (breath.get_moles(/datum/gas/oxygen) / safe_oxygen_max) * 10 // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/ratio = (breath.moles[/datum/gas/oxygen] / safe_oxygen_max) * 10
...
/obj/item/organ/lungs/proc/breathe_nitro(mob/living/carbon/breather, datum/gas_mixture/breath, nitro_pp, old_nitro_pp)
	if(nitro_pp < safe_nitro_min && !HAS_TRAIT(breather, TRAIT_NO_BREATHLESS_DAMAGE))
		...
		var/gas_breathed = handle_suffocation(breather, nitro_pp, safe_nitro_min, breath.get_moles(/datum/gas/nitrogen)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/gas_breathed = handle_suffocation(breather, nitro_pp, safe_nitro_min, breath.moles[/datum/gas/nitrogen])
		if(nitro_pp)
...
/obj/item/organ/lungs/proc/breathe_plasma(mob/living/carbon/breather, datum/gas_mixture/breath, plasma_pp, old_plasma_pp)
	...
	if(plasma_pp < safe_plasma_min && !HAS_TRAIT(breather, TRAIT_NO_BREATHLESS_DAMAGE))
		...
		var/gas_breathed = handle_suffocation(breather, plasma_pp, safe_plasma_min, breath.get_moles(/datum/gas/plasma)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/gas_breathed = handle_suffocation(breather, plasma_pp, safe_plasma_min, breath.moles[/datum/gas/plasma])
		if(plasma_pp)
...
/obj/item/organ/lungs/proc/too_much_plasma(mob/living/carbon/breather, datum/gas_mixture/breath, plasma_pp, old_plasma_pp)
	...
	var/ratio = (breath.get_moles(/datum/gas/plasma) / safe_plasma_max) * 10 // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/ratio = (breath.moles[/datum/gas/plasma] / safe_plasma_max) * 10
...
/obj/item/organ/lungs/proc/check_breath(datum/gas_mixture/breath, mob/living/carbon/human/breather)
	...
	*/ // APHELION EDIT REMOVAL END
	...
	breath_out.set_temperature(breath.return_temperature()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: breath_out.temperature = breath.temperature
...
/obj/item/organ/lungs/proc/breathe_gas_volume(datum/gas_mixture/breath, remove_id, exchange_id = null, volume = INFINITY)
	...
	*/ // APHELION EDIT REMOVAL END
	...
	// APHELION EDIT ADDITION END
	if(exchange_id)
		breath_out.adjust_moles(exchange_id, volume) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: breath_out.moles[exchange_id] += volume
	return volume
...
/obj/item/organ/lungs/proc/handle_breath_temperature(datum/gas_mixture/breath, mob/living/carbon/human/breather) // called by human/life, handles temperatures
	var/breath_temperature = breath.return_temperature() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/breath_temperature = breath.temperature
	...
	breath.set_temperature(breather.bodytemperature) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: breath.temperature = breather.bodytemperature
...
/obj/item/organ/lungs/ethereal/proc/consume_water(mob/living/carbon/breather, datum/gas_mixture/breath, h2o_pp, old_h2o_pp)
	var/gas_breathed = breath.get_moles(/datum/gas/water_vapor) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/gas_breathed = breath.moles[/datum/gas/water_vapor]
