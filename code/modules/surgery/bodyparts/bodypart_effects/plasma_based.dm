...
/datum/status_effect/grouped/bodypart_effect/plasma_based/tick(seconds_between_ticks)
	...
	if(environment.get_moles(/datum/gas/hypernoblium) >= 5) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(environment.moles[/datum/gas/hypernoblium] >= 5)
	...
	if(environment.get_moles(/datum/gas/oxygen) < 1) //Same threshhold that extinguishes fire // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(environment.moles[/datum/gas/oxygen] < 1) //Same threshhold that extinguishes fire
		return
