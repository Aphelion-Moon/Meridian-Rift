/obj/machinery/atmospherics/pipe/heat_exchanging
	...
	wake_on_pipeline_atmos = TRUE
	wake_on_turf_atmos = TRUE
...
/** Exchanges heat with the environment, sleeping after thermal convergence. */ // APHELION EDIT ADDITION - DOGMOS
/obj/machinery/atmospherics/pipe/heat_exchanging/process_atmos()
	var/environment_temperature = 0
	var/datum/gas_mixture/pipe_air = return_air()
	var/temperature_interacted = FALSE // APHELION EDIT ADDITION - DOGMOS
	...
	if(istype(local_turf))
		...
		//NOVA EDIT END
			...
			environment_temperature = local_turf.get_dogmos_blocked_temperature() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: environment_temperature = local_turf.temperature
		else
	...
	if(abs(environment_temperature-pipe_air.return_temperature()) > minimum_temperature_difference)
		...
		temperature_interacted = TRUE // APHELION EDIT ADDITION - DOGMOS
	...
	if(has_buckled_mobs())
		...
		var/avg_temp = (pipe_air.return_temperature() * hc + (heat_source.bodytemperature * buckled_mobs.len) * 3500) / (hc + (buckled_mobs ? buckled_mobs.len * 3500 : 0)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/avg_temp = (pipe_air.temperature * hc + (heat_source.bodytemperature * buckled_mobs.len) * 3500) / (hc + (buckled_mobs ? buckled_mobs.len * 3500 : 0))
...
/obj/machinery/atmospherics/pipe/heat_exchanging/process(seconds_per_tick)
	...
	*/ // APHELION EDIT REMOVAL END
	...
	if(!has_buckled_mobs())
		return
	var/heat_limit = 1000
	if(pipe_air.return_temperature() > heat_limit + 1) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(pipe_air.temperature > heat_limit + 1)
		for(var/mob/living/buckled_mob as anything in buckled_mobs)
			buckled_mob.apply_damage(seconds_per_tick * 2 * log(pipe_air.return_temperature() - heat_limit), BURN, BODY_ZONE_CHEST) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: buckled_mob.apply_damage(seconds_per_tick * 2 * log(pipe_air.temperature - heat_limit), BURN, BODY_ZONE_CHEST)
