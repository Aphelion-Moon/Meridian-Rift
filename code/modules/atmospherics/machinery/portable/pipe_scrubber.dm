...
/obj/machinery/portable_atmospherics/pipe_scrubber/proc/scrub()
	...
	var/transfer_moles = min(1, volume_rate / air_contents.return_volume()) * air_contents.total_moles() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/transfer_moles = min(1, volume_rate / air_contents.volume) * air_contents.total_moles()
