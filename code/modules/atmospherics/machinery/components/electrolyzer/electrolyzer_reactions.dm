...
/datum/gas_reaction/electrolyzer/h2o_conversion/react(datum/gas_mixture/air_mixture, working_power, list/electrolyzer_args = list())
	...
	var/proportion = min(air_mixture.get_moles(/datum/gas/water_vapor) * INVERSE(2), (2.5 * (working_power ** 2))) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/proportion = min(air_mixture.moles[/datum/gas/water_vapor] * INVERSE(2), (2.5 * (working_power ** 2)))
...
/datum/gas_reaction/electrolyzer/nob_conversion/react(datum/gas_mixture/air_mixture, working_power, list/electrolyzer_args = list())
	...
	*/ // APHELION EDIT REMOVAL END
	var/electrolysed = air_mixture.get_moles(/datum/gas/hypernoblium) * clamp(supermatter_power - POWER_PENALTY_THRESHOLD, 0, CRITICAL_POWER_PENALTY_THRESHOLD - POWER_PENALTY_THRESHOLD) / (CRITICAL_POWER_PENALTY_THRESHOLD - POWER_PENALTY_THRESHOLD) // APHELION EDIT ADDITION - DOGMOS
...
/datum/gas_reaction/electrolyzer/halon_generation/react(datum/gas_mixture/air_mixture, working_power, list/electrolyzer_args = list())
	...
	*/ // APHELION EDIT REMOVAL END
	...
	var/new_heat_capacity = air_mixture.heat_capacity()
	if(new_heat_capacity > MINIMUM_HEAT_CAPACITY)
		air_mixture.set_temperature(max(((air_mixture.return_temperature() * old_heat_capacity + energy_used) / new_heat_capacity), TCMB)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: air_mixture.temperature = max(((air_mixture.temperature * old_heat_capacity + energy_used) / new_heat_capacity), TCMB)
