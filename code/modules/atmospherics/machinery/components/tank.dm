...
/obj/machinery/atmospherics/components/tank/proc/fill_to_pressure(gastype, safety_margin = 0.5)
	...
	var/moles_to_add = (pressure_limit * air_contents.return_volume()) / (R_IDEAL_GAS_EQUATION * air_contents.return_temperature()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/moles_to_add = (pressure_limit * air_contents.volume) / (R_IDEAL_GAS_EQUATION * air_contents.temperature)

	air_contents.adjust_gas(gastype, moles_to_add)
	air_contents.archive()

/** Processes gas reactions and pressure damage, sleeping while the tank is stable. */ // APHELION EDIT ADDITION - DOGMOS
/obj/machinery/atmospherics/components/tank/process_atmos()
	...
	*/ // APHELION EDIT REMOVAL END
	...
	refresh_window()
	// APHELION EDIT ADDITION START - DOGMOS
	if(!reaction_occurred && !overpressure && !air_contents.dogmos_fusion_waiting(src))
		return PROCESS_KILL
// APHELION EDIT ADDITION END
...
/obj/machinery/atmospherics/components/tank/proc/merger_refresh_complete(datum/merger/merger, list/leaving_members, list/joining_members)
	...
	for(var/obj/machinery/atmospherics/components/tank/leaver as anything in leaving_members)
		...
		air_contents.set_volume(air_contents.return_volume() - leaver.volume) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: air_contents.volume -= leaver.volume
	...
	for(var/obj/machinery/atmospherics/components/tank/joiner as anything in joining_members)
		...
		air_contents.set_volume(air_contents.return_volume() + joiner.volume) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: air_contents.volume += joiner.volume
