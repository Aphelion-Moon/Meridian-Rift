...
/obj/machinery/atmospherics/components/binary/dp_vent_pump/process_atmos()
	...
	if(pump_direction) //input -> external
		...
		if(pressure_delta <= 0)
			return
		if(air1.return_temperature() <= 0) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(air1.temperature <= 0)
			return
		var/transfer_moles = (pressure_delta*environment.return_volume())/(air1.return_temperature() * R_IDEAL_GAS_EQUATION) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/transfer_moles = (pressure_delta*environment.volume)/(air1.temperature * R_IDEAL_GAS_EQUATION)
		...
		if(pressure_delta <= 0)
			return
		if(environment.return_temperature() <= 0) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(environment.temperature <= 0)
			return
		var/transfer_moles = (pressure_delta*air2.return_volume())/(environment.return_temperature() * R_IDEAL_GAS_EQUATION) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/transfer_moles = (pressure_delta*air2.volume)/(environment.temperature * R_IDEAL_GAS_EQUATION)
