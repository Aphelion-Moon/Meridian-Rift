...
/obj/machinery/atmospherics/components/binary/crystallizer/proc/check_temp_requirements()
	if(internal.return_temperature() >= selected_recipe.min_temp * MIN_DEVIATION_RATE && internal.return_temperature() <= selected_recipe.max_temp * MAX_DEVIATION_RATE) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(internal.temperature >= selected_recipe.min_temp * MIN_DEVIATION_RATE && internal.temperature <= selected_recipe.max_temp * MAX_DEVIATION_RATE)
		return TRUE
	return FALSE
...
/obj/machinery/atmospherics/components/binary/crystallizer/proc/inject_gases()
	...
	for(var/gas_type in selected_recipe.requirements)
		if(!contents.get_moles(gas_type)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!contents.moles[gas_type])
			continue
		if(internal.get_moles(gas_type) >= selected_recipe.requirements[gas_type] * 2) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(internal.moles[gas_type] >= selected_recipe.requirements[gas_type] * 2)
			continue
		internal.merge(contents.remove_specific(gas_type, contents.get_moles(gas_type) * gas_input)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: internal.merge(contents.remove_specific(gas_type, contents.moles[gas_type] * gas_input))

///Checks if the gases required are all inside
/obj/machinery/atmospherics/components/binary/crystallizer/proc/internal_check()
	var/gas_check = 0
	for(var/gas_type in selected_recipe.requirements)
		if(!internal.get_moles(gas_type)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!internal.moles[gas_type])
			return FALSE
		if(internal.get_moles(gas_type) >= selected_recipe.requirements[gas_type]) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(internal.moles[gas_type] >= selected_recipe.requirements[gas_type])
			gas_check++
...
/obj/machinery/atmospherics/components/binary/crystallizer/proc/heat_calculations()
	...
	*/ // APHELION EDIT REMOVAL END
	...
	if(internal.return_temperature() >= (median_temperature * MIN_DEVIATION_RATE) && internal.return_temperature() <= (median_temperature * MAX_DEVIATION_RATE)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(internal.temperature >= (median_temperature * MIN_DEVIATION_RATE) && internal.temperature <= (median_temperature * MAX_DEVIATION_RATE))
		quality_loss = max(quality_loss - progress_amount_to_quality, -85)

	internal.set_temperature(max(internal.return_temperature() + (selected_recipe.energy_release / internal.heat_capacity()), TCMB)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: internal.temperature = max(internal.temperature + (selected_recipe.energy_release / internal.heat_capacity()), TCMB)
	update_parents()
...
/obj/machinery/atmospherics/components/binary/crystallizer/proc/heat_conduction()
	...
	if(cooling_port.total_moles() > MINIMUM_MOLE_COUNT)
		if(internal.total_moles() > 0)
			var/coolant_temperature_delta = cooling_port.return_temperature() - internal.return_temperature() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/coolant_temperature_delta = cooling_port.temperature - internal.temperature
...
/obj/machinery/atmospherics/components/binary/crystallizer/process_atmos()
	...
	for(var/gas_type in selected_recipe.requirements)
		...
		if(internal.get_moles(gas_type) < amount_consumed) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(internal.moles[gas_type] < amount_consumed)
...
/obj/machinery/atmospherics/components/binary/crystallizer/ui_data()
	...
	if(internal.total_moles())
		...
		*/ // APHELION EDIT REMOVAL END
		...
		for(var/gasid in internal.get_gases()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: for(var/gasid in internal.moles)
			internal_gas_data.Add(list(list(
				"name"= cached_gas_name[gasid],
				"id" = cached_gas_id[gasid],
				"amount" = 0,
	...
	var/temperature
	if(internal.total_moles())
		temperature = internal.return_temperature() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: temperature = internal.temperature
	else
		temperature = 0
