...
/obj/machinery/atmospherics/components/trinary/mixer/Initialize(mapload)
	...
	air3.set_volume(300) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: air3.volume = 300
	airs[3] = air3
	register_context()
...
/obj/machinery/atmospherics/components/trinary/mixer/process_atmos()
	...
	var/general_transfer = (target_pressure - output_starting_pressure) * air3.return_volume() / R_IDEAL_GAS_EQUATION // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/general_transfer = (target_pressure - output_starting_pressure) * air3.volume / R_IDEAL_GAS_EQUATION
	...
	if(!node2_concentration)
		...
		if(air2.return_temperature() <= 0) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(air2.temperature <= 0)
			return
		transfer_moles2 = min(transfer_moles2, air2_moles)
		transfer_moles1 = 0
	else
		if(air1.return_temperature() <= 0 || air2.return_temperature() <= 0) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(air1.temperature <= 0 || air2.temperature <= 0)
			return
