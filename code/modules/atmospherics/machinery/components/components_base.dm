...
/obj/machinery/atmospherics/components/proc/relocate_airs(datum/gas_mixture/to_release)
	...
	for(var/i in 1 to device_type)
		...
		if(!nodes[i] || (istype(nodes[i], /obj/machinery/atmospherics/components/unary/portables_connector) && !portable_device_connected(i)) || !parents[i]) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!nodes[i] || (istype(nodes[i], /obj/machinery/atmospherics/components/unary/portables_connector) && !portable_device_connected(i)))
			if(!to_release)
				to_release = air
				continue
