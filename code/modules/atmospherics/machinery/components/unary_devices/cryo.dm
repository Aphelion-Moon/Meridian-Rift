...
/obj/machinery/cryo_cell/process_atmos()
	...
	if(!internal_connector.gas_connector.nodes[1] || !air1 || air1.total_moles() < CRYO_MIN_GAS_MOLES) // Turn off if the machine won't work. // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!internal_connector.gas_connector.nodes[1] || !air1 || !air1.moles.len || air1.total_moles() < CRYO_MIN_GAS_MOLES) // Turn off if the machine won't work.
		set_on(FALSE)
	...
	*/ // APHELION EDIT REMOVAL END
	...
	if(!QDELETED(occupant))
		...
		var/temperature_delta = air1.return_temperature() - mob_occupant.bodytemperature // The only semi-realistic thing here: share temperature between the cell and the occupant. // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/temperature_delta = air1.temperature - mob_occupant.bodytemperature // The only semi-realistic thing here: share temperature between the cell and the occupant.

		if(ishuman(mob_occupant))
			var/mob/living/carbon/human/H = mob_occupant
			cold_protection = H.get_cold_protection(air1.return_temperature()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: cold_protection = H.get_cold_protection(air1.temperature)
		...
		if(abs(temperature_delta) > 1)
			...
			air1.set_temperature(clamp(air1.return_temperature() - heat / air_heat_capacity, TCMB, MAX_TEMPERATURE)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: air1.temperature = clamp(air1.temperature - heat / air_heat_capacity, TCMB, MAX_TEMPERATURE)
...
/obj/machinery/cryo_cell/handle_internal_lifeform(mob/lifeform_inside_me, breath_request)
	...
	var/datum/gas_mixture/air1 = internal_connector.gas_connector.airs[1]
	var/breath_percentage = breath_request / air1.return_volume() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/breath_percentage = breath_request / air1.volume
...
/obj/machinery/cryo_cell/return_temperature()
	...
	return internal_air.total_moles() > CRYO_MIN_GAS_MOLES ? internal_air.return_temperature() : ..() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: return internal_air.total_moles() > CRYO_MIN_GAS_MOLES ? internal_air.temperature : ..()
...
/obj/machinery/cryo_cell/ui_data()
	...
	var/datum/gas_mixture/air1 = internal_connector.gas_connector.airs[1]
	.["cellTemperature"] = air1.return_temperature() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: .["cellTemperature"] = air1.temperature
