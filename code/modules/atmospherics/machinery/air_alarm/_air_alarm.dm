...
/obj/machinery/airalarm/proc/check_enviroment()
	...
	if(isnull(environment))
		return
	check_danger(our_turf, environment, environment.return_temperature()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: check_danger(our_turf, environment, environment.temperature)
...
/obj/machinery/airalarm/ui_data(mob/user)
	...
	var/temp = environment.return_temperature() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/temp = environment.temperature
	...
	if(total_moles)
		for(var/gas_path, moles in environment.get_moles_list()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: for(var/gas_path, moles in environment.moles)
...
/obj/machinery/airalarm/proc/check_danger(turf/location, datum/gas_mixture/environment, exposed_temperature)
	...
	if(total_moles)
		var/list/cached_gas_info = GLOB.meta_gas_info
		for(var/datum/gas/gas_path as anything in cached_gas_info[META_GAS_ID])
			var/moles = environment.get_moles(gas_path) || 0 // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/moles = environment.moles[gas_path] || 0
