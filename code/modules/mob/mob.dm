...
GAME_VERB_PROC(/mob, Cell, "Cell", "Admin")
	...
	t += "[span_danger("Temperature: [environment.return_temperature()] ")]\n" // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: t += "[span_danger("Temperature: [environment.temperature] ")]\n"
	var/list/cached_gas_name = GAS_META[META_GAS_NAME]
	for(var/gas_id, gas_moles in environment.get_moles_list()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: for(var/gas_id, gas_moles in environment.moles)
		if(gas_moles)
