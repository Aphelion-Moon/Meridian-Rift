...
/mob/living/basic/regal_rat/handle_environment(datum/gas_mixture/environment)
	. = ..()
	if(stat == DEAD || isnull(environment) || !environment.get_moles(/datum/gas/miasma)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(stat == DEAD || isnull(environment) || isnull(environment.moles[/datum/gas/miasma]))
		return
	var/miasma_percentage = environment.get_moles(/datum/gas/miasma) / environment.total_moles() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/miasma_percentage = environment.moles[/datum/gas/miasma] / environment.total_moles()
