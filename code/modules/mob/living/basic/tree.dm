...
/mob/living/basic/tree/Life(seconds_per_tick = SSMOBS_DT)
	...
	if(!our_turf.air || !our_turf.air.get_moles(/datum/gas/carbon_dioxide)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!our_turf.air || !our_turf.air.moles[/datum/gas/carbon_dioxide])
		return
	var/datum/gas_mixture/our_air = our_turf.air
	var/co2 = our_air.get_moles(/datum/gas/carbon_dioxide) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/co2 = our_air.moles[/datum/gas/carbon_dioxide]
