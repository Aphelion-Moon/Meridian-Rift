...
/obj/machinery/power/turbine/Initialize(mapload, gas_theoretical_volume)
	...
	machine_gasmix = new
	machine_gasmix.set_volume(gas_theoretical_volume) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: machine_gasmix.volume = gas_theoretical_volume
...
/obj/machinery/power/turbine/proc/transfer_gases(datum/gas_mixture/input_mix, datum/gas_mixture/output_mix, work_amount_to_remove, intake_size = 1)
	...
	//compute work done
	var/work_done = QUANTIZE(transferred_gases.total_moles()) * R_IDEAL_GAS_EQUATION * transferred_gases.return_temperature() * log((transferred_gases.return_volume() * PRESSURE_MAX(transferred_gases.return_pressure())) / (output_mix.return_volume() * output_pressure)) * TURBINE_WORK_CONVERSION_MULTIPLIER // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/work_done = QUANTIZE(transferred_gases.total_moles()) * R_IDEAL_GAS_EQUATION * transferred_gases.temperature * log((transferred_gases.volume * PRESSURE_MAX(transferred_gases.return_pressure())) / (output_mix.volume * output_pressure)) * TURBINE_WORK_CONVERSION_MULTIPLIER
...
/obj/machinery/power/turbine/inlet_compressor/proc/compress_gases()
	...
	return input_turf_mixture.return_temperature() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: return input_turf_mixture.temperature
...
/obj/machinery/power/turbine/core_rotor/process(seconds_per_tick)
	...
	if(!ejected_gases) //output turf was blocked with high pressure/temperature gases or by some structure so no power generated
		...
		produced_energy = 0
		return
	var/work_done =  QUANTIZE(ejected_gases.total_moles()) * R_IDEAL_GAS_EQUATION * ejected_gases.return_temperature() * log(compressor.compressor_pressure / PRESSURE_MAX(ejected_gases.return_pressure())) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/work_done =  QUANTIZE(ejected_gases.total_moles()) * R_IDEAL_GAS_EQUATION * ejected_gases.temperature * log(compressor.compressor_pressure / PRESSURE_MAX(ejected_gases.return_pressure()))
