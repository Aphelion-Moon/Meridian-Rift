...
/obj/item/fish/proc/proper_environment(temp_range_min = required_temperature_min, temp_range_max = required_temperature_max)
	...
	if(safe_air_limits && !mixture.check_gases(safe_air_limits))
		return FALSE
	if(!ISINRANGE(mixture.return_temperature(), required_temperature_min, required_temperature_max)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!ISINRANGE(mixture.temperature, required_temperature_min, required_temperature_max))
		return FALSE
