...
/obj/item/fish/dolphish/do_fish_process(seconds_per_tick)
	...
	if(turf_gasmix.get_moles(/datum/gas/water_vapor) >= 5) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(turf_gasmix.moles[/datum/gas/water_vapor] >= 5)
		patience_reduction *= 0.5
	if(turf_gasmix.get_moles(/datum/gas/nitrous_oxide) >= 5) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(turf_gasmix.moles[/datum/gas/nitrous_oxide] >= 5)
		patience_reduction *= 0.25
	if(turf_gasmix.get_moles(/datum/gas/healium) >= 5) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(turf_gasmix.moles[/datum/gas/healium] >= 5)
...
/obj/item/fish/gullion/suicide_act(mob/living/user)
	...
	var/oxygen_in_air = environment.get_moles(/datum/gas/oxygen) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/oxygen_in_air = locate(/datum/gas/oxygen) in environment.moles
