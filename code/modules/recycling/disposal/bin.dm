...
/obj/machinery/disposal/bin/process(seconds_per_tick)
	...
	if(!env.return_temperature()) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!env.temperature)
		return
	var/pressure_delta = (SEND_PRESSURE*1.01) - air_contents.return_pressure()

	var/transfer_moles = 0.05 * seconds_per_tick * (pressure_delta*air_contents.return_volume())/(env.return_temperature() * R_IDEAL_GAS_EQUATION) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/transfer_moles = 0.05 * seconds_per_tick * (pressure_delta*air_contents.volume)/(env.temperature * R_IDEAL_GAS_EQUATION)
