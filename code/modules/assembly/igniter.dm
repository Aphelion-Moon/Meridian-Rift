...
/obj/item/assembly/igniter/condenser/activate()
	...
	if(location)
		...
		enviro.set_temperature(clamp(min(ROOM_TEMP, enviro.return_temperature() * 0.85), MIN_FREEZE_TEMP, MAX_FREEZE_TEMP)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: enviro.temperature = clamp(min(ROOM_TEMP, enviro.temperature * 0.85), MIN_FREEZE_TEMP, MAX_FREEZE_TEMP)
