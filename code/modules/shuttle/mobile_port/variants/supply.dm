...
/obj/docking_port/mobile/supply/proc/refill_air()
	for(var/area/shuttle/shuttle_area as anything in shuttle_areas)
		for(var/turf/open/floor/shuttle_floor in shuttle_area.get_turfs_from_all_zlevels())
			...
			shuttle_floor.set_temperature(initial(shuttle_floor.temperature)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: shuttle_floor.temperature = initial(shuttle_floor.temperature)
