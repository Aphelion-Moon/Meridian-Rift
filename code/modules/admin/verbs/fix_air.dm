...
ADMIN_VERB_AND_CONTEXT_MENU(fix_air, R_ADMIN, "Fix Air", "Fixes air in a specified radius.", ADMIN_CATEGORY_GAME, /turf/open)
	...
	for(var/turf/open/valid_range_turf in range(range,locale))
		...
		valid_range_turf.set_temperature(initial(valid_range_turf.temperature)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: valid_range_turf.temperature = initial(valid_range_turf.temperature)
