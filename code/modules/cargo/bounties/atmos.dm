...
/datum/bounty/item/atmospherics/applies_to(obj/applied_obj)
	...
	if(!our_mix.get_moles(gas_type)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!our_mix.moles[gas_type])
		return FALSE
	return our_mix.get_moles(gas_type) >= moles_required // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: return our_mix.moles[gas_type] >= moles_required
...
/datum/bounty/item/atmospherics/contribution_amount(obj/shipped)
	...
	return our_mix.get_moles(gas_type) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: return our_mix.moles[gas_type]
