...
/datum/experimentor_result_handler/scan/heat/handle_malfunctions(obj/machinery/rnd/experimentor/machine, obj/item/exp_on)
	...
	if(prob(EFFECT_PROB_VERYLOW * malf_chance))
		...
		if(env)
			...
			env.set_temperature(min((env.return_temperature() * heat_capacity + 100000) / heat_capacity, 1000)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: env.temperature = min((env.temperature * heat_capacity + 100000) / heat_capacity, 1000)
...
/datum/experimentor_result_handler/scan/cold/handle_malfunctions(obj/machinery/rnd/experimentor/machine, obj/item/exp_on)
	...
	if(prob(EFFECT_PROB_VERYLOW * malf_chance))
		...
		if(env)
			var/heat_capacity = max(env.heat_capacity(), 1)
			env.set_temperature(max((env.return_temperature() * heat_capacity - 75000) / heat_capacity, TCMB)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: env.temperature = max((env.temperature * heat_capacity - 75000) / heat_capacity, TCMB)
		machine.air_update_turf(FALSE, FALSE)
