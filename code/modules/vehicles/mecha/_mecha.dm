...
/obj/vehicle/sealed/mecha/proc/process_internal_damage_effects(seconds_per_tick)
	if(internal_damage & MECHA_INT_FIRE)
		...
		if(cabin_air && cabin_sealed && cabin_air.return_volume()>0)
			...
			cabin_air.set_temperature(min(6000+T0C, cabin_air.return_temperature()+rand(5,7.5)*seconds_per_tick)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: cabin_air.temperature = min(6000+T0C, cabin_air.temperature+rand(5,7.5)*seconds_per_tick)
...
/obj/vehicle/sealed/mecha/proc/process_cabin_air(seconds_per_tick)
	if(!(internal_damage & MECHA_INT_TEMP_CONTROL) && cabin_air && cabin_air.return_volume() > 0)
		...
		var/required_energy = abs(T20C - cabin_air.return_temperature()) * heat_capacity // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/required_energy = abs(T20C - cabin_air.temperature) * heat_capacity
		...
		if(delta_temperature)
			...
			*/ // APHELION EDIT REMOVAL END
			...
			// APHELION EDIT ADDITION END
				...
				cabin_air.set_temperature(cabin_air.return_temperature() - delta_temperature) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: cabin_air.temperature -= delta_temperature
