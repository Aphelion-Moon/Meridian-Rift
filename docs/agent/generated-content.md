...
Dogmos bindings and native contract defines are governed by [native artifacts](native-artifacts.md). `code/__DEFINES/dogmos_bindings.dm` and `code/__DEFINES/dogmos_contract.dm` are downstream products of the source-bound Rust build. They must match the Windows/Linux in-process manifests and are installed atomically.
