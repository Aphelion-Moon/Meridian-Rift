...
[menuTheme.ts](../../menuTheme.ts) reads each theme definition's optional `lobby`
settings: `heading` defaults to `NETWORK ACCESS`, and `layout: 'custom'` opts out
of the shared instrument layout. `AphelionLobbyMenu` uses the same
...
New palette themes inherit the instrument menu automatically. Add menu-specific
paint to `_instrument-menus.scss` or `_signal-menus.scss` only when needed; use
`layout: 'custom'` when providing a separate layout partial. The final shared
theme finishes enter once through TGUI's `_finishes.scss`, after these menus.
