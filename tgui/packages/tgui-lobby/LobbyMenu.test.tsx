...
function makeServerState(overrides: Partial<ServerState> = {}): ServerState {
  return {
    ...
    progressCurrent: 25,
    progressTotal: 100,
    initializedSubsystems: 12,
    totalSubsystems: 26,
    startupSubsystem: 'Mapping',
...
describe('LobbyMenu MeridianOS integration', () => {
  it.each(
  ...
  )('preserves whitelist gating and actions with theme %s', (meridianTheme) => {
    render(<LobbyMenu />);
    emit(
      'init',
      makeServerState({
    ...
    expect(whitelistButton.classList.contains('menu_button--centered')).toBe(
      true,
    );
    ...
    expect(sendMessage).toHaveBeenCalledWith('action', {
      action: 'get_whitelisted',
    });
    fireEvent.click(screen.getByRole('button', { name: 'SWAP SERVERS' }));
    expect(sendMessage).toHaveBeenCalledWith('action', {
      action: 'server_swap',
    });

    emit('state', { whitelistGate: 'checking', canSwapServers: false });
    expect(screen.getByText('CHECKING WHITELIST...')).toBeTruthy();
    expect(screen.queryByRole('button', { name: 'SWAP SERVERS' })).toBeNull();
    expect(screen.queryByText('TEST PLAYER')).toBeNull();

    emit('state', { whitelistGate: 'unavailable' });
    expect(screen.getByText(/Whitelist database unreachable/)).toBeTruthy();
    expect(screen.queryByText('TEST PLAYER')).toBeNull();

    emit('state', { whitelistGate: 'open' });
    expect(screen.queryByText(/Whitelist database unreachable/)).toBeNull();
  ...
  it('keeps the same boot loader and RAF when only the theme changes', () => {
    ...
    const loader = screen.getByRole('progressbar', {
      name: 'Estimated startup progress',
    });
    ...
    expect(
      screen.getByRole('progressbar', { name: 'Estimated startup progress' }),
    ).toBe(loader);
