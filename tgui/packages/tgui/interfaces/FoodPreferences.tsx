      ...
      <Window.Content scrollable>
        <Section
          title={
            <Tooltip
              position="bottom"
              content={`You HAVE to pick at least ${limits.min_toxic} TOXIC and ${limits.min_disliked} DISLIKED foods. You can have a maximum of ${limits.max_liked} LIKED foods.`}
            >
              <Box inline>
                <Icon name="circle-question" mr="0.5em" />
                {invalid ? (
                  <Box as="span" color="#bd2020">
                    ...
                    {counts.disliked < limits.min_disliked
                      ? `${counts.disliked}/${limits.min_disliked} Disliked`
                      : `${counts.toxic}/${limits.min_toxic} Toxic`}
                  </Box>
                ) : (
                  <Box as="span" color="green">
                    Preferences are Valid! | <b>{counts.liked}</b>/
                    {limits.max_liked} Liked
                  </Box>
                )}
              </Box>
            </Tooltip>
              ...
              <Button
                ...
                tooltip={
                  <>
                    ...
                    likes, even if you don&apos;t have its food type liked here!
                  </>
                }
              >
        ...
        >
          ...
          <Box style={{ columns: '30em' }}>
            {Object.entries(food_types).map((element) => {
              ...
              return (
                // Keeps a food's card from being split across two columns.
                <Box key={foodName} style={{ breakInside: 'avoid' }}>
                  <Section
                    title={
                      <>
                        {foodName}
                  ...
                  >
                    ...
                    <FoodButton
                      ...
                      tooltip={
                        !obscure_food_types.includes(foodName) &&
                        counts.liked >= limits.max_liked
                          ? `You currently have too many liked foods, you cannot have more than ${limits.max_liked} foods that are not obscure!`
