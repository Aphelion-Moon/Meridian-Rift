...
const CustomPaletteSection = ({
  ...
  return (
    <Section title={title}>
      <Stack style={{ flexWrap: 'wrap', gap: '0.5rem' }}>
        {colors.map((color, i) => {
          ...
          const button = (
            <Button
              ...
              height="2em"
              disabled={disabled}
              tooltip={disabled ? disabledColorTooltip : undefined}
  ...
  return (
    <Stack vertical>
      ...
      <Stack.Item>
        <CustomPaletteSection
          ...
          onClickColor={(_selected, _rightClick, index) => {
            act('selectCustomColor', { color: customPalette[index] });
          }}
          ...
          disabledColors={savedColors.map(
      ...
      <Stack.Item color="label">
        Saved to your account. Right-click for options; Delete also removes
        custom colors.
      </Stack.Item>
    </Stack>
  );
