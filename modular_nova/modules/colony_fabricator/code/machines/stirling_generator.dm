...
/obj/machinery/power/stirling_generator/process_atmos()
	...
	var/gas_temperature_delta = hot_air_from_pipe.return_temperature() - environment.return_temperature() // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/gas_temperature_delta = hot_air_from_pipe.temperature - environment.temperature
	...
	hot_air_from_pipe.set_temperature(max(hot_air_from_pipe.return_temperature() - (cooling_heat_amount / input_capacity), TCMB)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: hot_air_from_pipe.temperature = max(hot_air_from_pipe.temperature - (cooling_heat_amount / input_capacity), TCMB)
