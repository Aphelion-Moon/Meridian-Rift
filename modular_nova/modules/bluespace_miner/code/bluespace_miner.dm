...
/obj/machinery/bluespace_miner/proc/update_mining_stat()
	...
	if(environment.return_temperature() >= T20C) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(environment.temperature >= T20C)
...
/obj/machinery/bluespace_miner/process()
	...
	*/ // APHELION EDIT REMOVAL END
	merger.set_moles(/datum/gas/carbon_dioxide, MOLES_CELLSTANDARD) // APHELION EDIT ADDITION - DOGMOS
	if(obj_flags & EMAGGED)
		/* // APHELION EDIT REMOVAL START - DOGMOS
		merger.assert_gas(/datum/gas/tritium)
		merger.moles[/datum/gas/tritium] = MOLES_CELLSTANDARD
		*/ // APHELION EDIT REMOVAL END
		merger.set_moles(/datum/gas/tritium, MOLES_CELLSTANDARD) // APHELION EDIT ADDITION - DOGMOS

	merger.set_temperature(T20C + gas_temp) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: merger.temperature = (T20C + gas_temp)
	var/turf/src_turf = get_turf(src)
