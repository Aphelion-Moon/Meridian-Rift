...
| Setting                             | Default                                             |
| ----------------------------------- | --------------------------------------------------- |
| Account base theme                  | Aphelion (`meridian_aphelion`)                      |
...
During `startup`, the lobby shows the boot terminal and hides title artwork. The subsystem still publishes a configured loading splash or the transparent loading placeholder as its initial image. Title artwork becomes visible after the lobby leaves `startup`, using the image published at that point; changing the title defaults does not replace the loading sequence.
...
The picker presents these choices in order:

| Display name | Stored ID                |
| ------------ | ------------------------ |
| Aphelion     | `meridian_aphelion`       |
| Classic      | `meridian_classic`        |
| Electra      | `meridian_electra`        |
| Vector       | `meridian_vector`         |
| Synapse      | `meridian_synapse`        |
| Highline     | `meridian_highline`       |
| Hephaestus   | `meridian_hephaestus`     |
| Diagnostic   | `meridian_diagnostic`     |
| Augmentation | `meridian_augmentation`   |
| Hotline      | `meridian_hotline`        |
| Cyberpunk    | `meridian_cyberpunk`      |
| Scavenger    | `meridian_scavenger`      |
| Wastelander  | `meridian_wastelander`    |
| Shadowbroker | `meridian_shadowbroker`   |
| Foundry      | `meridian_foundry`        |

Classic uses the original Nanotrasen component styling without the Meridian console layer. Saved preferences using retired IDs fall back to Aphelion until the player selects a theme again; there is no migration for the renamed IDs.
...
The lobby uses the account base theme. Specialty interfaces retain their authored appearance; their gear still updates the account preference used by ordinary windows. The resolver maps upstream `nanotrasen` and `ntos` requests to Electra; the preference picker sends canonical IDs from the table above.

In a development build, open the Kitchen Sink with F11 or the title-bar bug button. Its skin controls preview a window-local override. **Inherit** clears that override, and closing the window discards it. The component examples and loader preview use the same production theme components.
...
| [meridianos/](../../../tgui/packages/tgui/styles/meridianos/)                                                                                                                                                   | Shared theme tokens, components, loaders, decoration, accessibility rules and stylesheet entrypoints |
| [lobby meridianos/](../../../tgui/packages/tgui-lobby/styles/meridianos/)                                                                                                                                                            | Lobby layout profiles, artwork integration and decorative scanline layer                             |

`meridianos/_shared.scss` contains the common font/token/theme imports. `_index.scss` assembles ordinary TGUI styling; the lobby's `styles/meridianos/_meridian.scss` assembles its styling. `_preferences.scss` and `_titlebar.scss` load the upstream sheets before adding module-specific rules. Keep their cascade positions when moving imports.
...
```sh
bun run tgui:tsc
bun test packages/tgui/styles/meridianos/tests packages/tgui/layouts packages/tgui/interfaces/common packages/tgui/interfaces/TitleScreenManager.test.tsx packages/tgui-lobby
bun run tgui:build
```
