/obj/machinery/power/rbmk2/process(seconds_per_tick)
	var/turf/turf_loc = loc
	if(!istype(turf_loc))
		return //wat

	if(!stored_rod)
		return

	var/datum/gas_mixture/rod_mix = stored_rod.air_contents
	if(!rod_mix) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!rod_mix || !rod_mix.moles)
		return

	var/rod_mix_pressure = rod_mix.return_pressure()
	var/rod_mix_heat_capacity = rod_mix.heat_capacity()

	if(!active) //We're turned off.
		meltdown = FALSE //Sometimes, this thing can be set to inactive due to running out of gas and other memes, thus this is fine to exist and is totally not a bandaid solution to potential future fuckery.
		return

	var/amount_to_consume = (gas_consumption_base + (rod_mix.return_temperature()/1000)*gas_consumption_heat) * clamp(1 - (rod_mix_pressure - stored_rod.pressure_limit*0.5)/stored_rod.pressure_limit*0.5, 0.25, 1) * seconds_per_tick * 0.5 // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: var/amount_to_consume = (gas_consumption_base + (rod_mix.temperature/1000)*gas_consumption_heat) * clamp(1 - (rod_mix_pressure - stored_rod.pressure_limit*0.5)/stored_rod.pressure_limit*0.5, 0.25, 1) * seconds_per_tick * 0.5
	if(!amount_to_consume)
		return
	amount_to_consume *= (overclocked ? 1.25 : 1)*(0.75 + power_efficiency*0.25)*(obj_flags & EMAGGED ? 10 : 1)*seconds_per_tick*0.5

	//Remove gas from the rod to be processed.
	/* // APHELION EDIT REMOVAL START - DOGMOS
	rod_mix.assert_gas(/datum/gas/tritium)
	*/ // APHELION EDIT REMOVAL END
	var/datum/gas_mixture/consumed_mix = rod_mix.remove_specific(/datum/gas/tritium, amount_to_consume)

	if(!consumed_mix)
		toggle_active(desired_state = FALSE)
		return

	//Do power generation here.
	/* // APHELION EDIT REMOVAL START - DOGMOS
	if(consumed_mix.moles && consumed_mix.moles[/datum/gas/tritium])
		consumed_mix.assert_gas(/datum/gas/tritium)
		last_tritium_consumption = consumed_mix.moles[/datum/gas/tritium]
	*/ // APHELION EDIT REMOVAL END
	// APHELION EDIT ADDITION START - DOGMOS
	if(consumed_mix.get_moles(/datum/gas/tritium))
		last_tritium_consumption = consumed_mix.get_moles(/datum/gas/tritium)
		// APHELION EDIT ADDITION END
		last_power_generation = last_tritium_consumption * power_efficiency * base_power_generation * (overclocked ? 0.9 : 1) * seconds_per_tick * 0.5 //Overclocked consumes more, but generates less.
		//This is where the fun begins.
		// https://www.desmos.com/calculator/ffcsaaftzz
		last_power_generation *= (1 + max(0, (rod_mix.return_temperature() - T0C)/1500)**1.4)*(0.75 + (amount_to_consume/gas_consumption_base)*0.25) * seconds_per_tick * 0.5 // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: last_power_generation *= (1 + max(0, (rod_mix.temperature - T0C)/1500)**1.4)*(0.75 + (amount_to_consume/gas_consumption_base)*0.25) * seconds_per_tick * 0.5

		var/range_cap = ceil(GAS_REACTION_MAXIMUM_RADIATION_PULSE_RANGE * 0.5)
		if(meltdown)
			last_radiation_pulse = min(last_power_generation*0.002, range_cap) //Double the rads, double the fun.
		else
			last_radiation_pulse = min(last_power_generation*0.001, range_cap)

		var/insulation_threshold_math = (range_cap - last_radiation_pulse) / range_cap

		//The LOWER the insulation_threshold, the stronger the radiation can penetrate.
		//Values closer to the maximum range penetrate the most.
		//Don't bother making radiation if it isn't significiant enough.
		if(insulation_threshold_math <= RAD_LIGHT_INSULATION)
			if(meltdown)
				insulation_threshold_math = max(insulation_threshold_math - 0.25, RAD_FULL_INSULATION) //Go as low as possible. Nothing is safe from the RBMK.
			else
				insulation_threshold_math = max(insulation_threshold_math, RAD_EXTREME_INSULATION) //Don't go under RAD_EXTREME_INSULATION
			radiation_pulse(src, last_radiation_pulse, threshold = insulation_threshold_math)

		if(power && powernet && last_power_generation)
			src.add_avail(min(last_power_generation, max_power_generation*10))
		consumed_mix.remove_specific(/datum/gas/tritium, last_tritium_consumption*0.50) //50% of used tritium gets deleted. The rest gets thrown into the air.
		var/our_heat_capacity = consumed_mix.heat_capacity()
		if(our_heat_capacity > 0)
			var/temperature_mod = last_power_generation >= max_power_generation ? 4 : 1
			/* // APHELION EDIT REMOVAL START - DOGMOS
			consumed_mix.assert_gas(/datum/gas/goblin)
			consumed_mix.moles[/datum/gas/goblin] += last_tritium_consumption*goblin_multiplier * seconds_per_tick * 0.5
			consumed_mix.temperature += (temperature_mod-rand())*8 + (16000/our_heat_capacity)*(overclocked ? 2 : 1)*power_efficiency*temperature_mod*0.5*(1/(vent_pressure/200))*seconds_per_tick*0.5
			consumed_mix.temperature = clamp(consumed_mix.temperature, 5, 0xFFFFFF)
			*/ // APHELION EDIT REMOVAL END
			// APHELION EDIT ADDITION START - DOGMOS
			consumed_mix.adjust_moles(/datum/gas/goblin, last_tritium_consumption*goblin_multiplier * seconds_per_tick * 0.5)
			consumed_mix.set_temperature(consumed_mix.return_temperature() + ((temperature_mod-rand())*8 + (16000/our_heat_capacity)*(overclocked ? 2 : 1)*power_efficiency*temperature_mod*0.5*(1/(vent_pressure/200))*seconds_per_tick*0.5))
			consumed_mix.set_temperature(clamp(consumed_mix.return_temperature(), 5, 0xFFFFFF))
		// APHELION EDIT ADDITION END

		if(rod_mix_pressure >= stored_rod.pressure_limit*(1 + rand()*0.25)) //Pressure friction penalty.
			/* // APHELION EDIT REMOVAL START - DOGMOS
			rod_mix.temperature += (min(rod_mix_pressure/stored_rod.pressure_limit, 4) - 1) * (3/rod_mix_heat_capacity)*seconds_per_tick*0.5
			rod_mix.temperature = clamp(rod_mix.temperature, 5, 0xFFFFFF)
			*/ // APHELION EDIT REMOVAL END
			// APHELION EDIT ADDITION START - DOGMOS
			rod_mix.set_temperature(rod_mix.return_temperature() + ((min(rod_mix_pressure/stored_rod.pressure_limit, 4) - 1) * (3/rod_mix_heat_capacity)*seconds_per_tick*0.5))
			rod_mix.set_temperature(clamp(rod_mix.return_temperature(), 5, 0xFFFFFF))
	// APHELION EDIT ADDITION END
	else
		toggle_active(desired_state = FALSE)

	//The gases that we consumed go into the buffer, to be released in the air.
	buffer_gases.merge(consumed_mix)

	//Share the remaining temperature with the rod mix itself.
	transfer_rod_temperature(buffer_gases)

	if(SPT_PROB(2.5, seconds_per_tick))
		playsound(src, 'sound/misc/metal_creak.ogg', 50, TRUE, extrarange = -3)

	if(!jammed && safety && (last_power_generation >= safeties_max_power_generation || rod_mix_pressure > stored_rod.pressure_limit*0.9)) //The safety system is very strict. Nanotrasen wants employees to feel safe!
		var/health_percent = atom_integrity/max_integrity
		var/jam_chance = 80 - (health_percent * 100) - (venting ? 0 : 40)
		if(jam_chance > 0 && SPT_PROB(jam_chance*0.5, seconds_per_tick))
			jam(desired_state = TRUE)
		else
			toggle_active(null, FALSE)
			take_damage(3, armour_penetration = 100)
			src.Shake(duration=0.5 SECONDS)

	if(active && rod_mix.return_temperature() > stored_rod.temperature_limit || last_power_generation > max_power_generation*(1.1 + rand()) ) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(active && rod_mix.temperature > stored_rod.temperature_limit || last_power_generation > max_power_generation*(1.1 + rand()) )
		if(!meltdown)
			log_game("[src] triggered a meltdown at [AREACOORD(turf_loc)]")
			investigate_log("triggered a meltdown at [AREACOORD(turf_loc)]", INVESTIGATE_ENGINE)
			meltdown = TRUE
		var/chosen_sound = pick(
			'modular_nova/modules/RBMK2/sounds/failure01.ogg',
			'modular_nova/modules/RBMK2/sounds/failure02.ogg',
			'modular_nova/modules/RBMK2/sounds/failure03.ogg',
			'modular_nova/modules/RBMK2/sounds/failure04.ogg',
		)
		playsound(src, chosen_sound, 50, TRUE, extrarange = -3)
		take_damage(2, armour_penetration = 100) //Lasts 5 minutes. Probably less due to other factors.
		src.Shake(duration = 0.5 SECONDS)
	else if(meltdown && rod_mix.return_temperature() <= stored_rod.temperature_limit*0.75 && last_power_generation <= max_power_generation*0.5) //Hard to get out of a meltdown. // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: else if(meltdown && rod_mix.temperature <= stored_rod.temperature_limit*0.75 && last_power_generation <= max_power_generation*0.5) //Hard to get out of a meltdown.
		meltdown = FALSE
	update_appearance(UPDATE_ICON)
	return TRUE

/obj/machinery/power/rbmk2/process_atmos() //Only turf air related stuff is handled here.
	var/turf/turf_loc = loc
	if(!istype(turf_loc))
		return //wat

	var/datum/gas_mixture/turf_air = turf_loc.return_air()

	if(stored_rod && meltdown)
		var/meltdown_multiplier = jammed ? 1 : 0.1 //Tried removing a rod during a meltdown.
		meltdown_multiplier *= last_power_generation/max_power_generation //It just gets worse.
		var/datum/gas_mixture/rod_mix = stored_rod.air_contents
		var/rod_mix_heat_capacity = rod_mix.heat_capacity()
		if(rod_mix_heat_capacity > 0)
			/* // APHELION EDIT REMOVAL START - DOGMOS
			rod_mix.temperature += (rod_mix.temperature*0.02*rand() + (8000/rod_mix_heat_capacity)*(overclocked ? 2 : 1))*meltdown_multiplier //It's... it's not shutting down!
			rod_mix.temperature = clamp(rod_mix.temperature, 5, 0xFFFFFF)
		var/ionize_air_amount = min( (0.5 + rod_mix.temperature/2000) * meltdown_multiplier, 5) //For every 2000 kelvin. Capped at 5 tiles.
			*/ // APHELION EDIT REMOVAL END
			// APHELION EDIT ADDITION START - DOGMOS
			rod_mix.set_temperature(rod_mix.return_temperature() + ((rod_mix.return_temperature()*0.02*rand() + (8000/rod_mix_heat_capacity)*(overclocked ? 2 : 1))*meltdown_multiplier)) //It's... it's not shutting down!
			rod_mix.set_temperature(clamp(rod_mix.return_temperature(), 5, 0xFFFFFF))
		var/ionize_air_amount = min( (0.5 + rod_mix.return_temperature()/2000) * meltdown_multiplier, 5) //For every 2000 kelvin. Capped at 5 tiles.
		// APHELION EDIT ADDITION END
		var/ionize_air_range = ceil(ionize_air_amount)
		var/total_ion_amount = 0
		for(var/turf/ion_turf as anything in RANGE_TURFS(ionize_air_range, turf_loc))
			if(!prob(80)) //Atmos optimization.
				continue
			var/datum/gas_mixture/ion_turf_mix = ion_turf.return_air()
			if(!ion_turf_mix || !ion_turf_mix.get_moles(/datum/gas/oxygen)) // APHELION EDIT CHANGE - DOGMOS - ORIGINAL: if(!ion_turf_mix || !ion_turf_mix.moles || !ion_turf_mix.moles[/datum/gas/oxygen] || !ion_turf_mix.moles[/datum/gas/oxygen])
				continue
			/* // APHELION EDIT REMOVAL START - DOGMOS
			ion_turf_mix.assert_gas(/datum/gas/oxygen)
			var/gas_to_convert = max(0, min(ionize_air_amount, ion_turf_mix.moles[/datum/gas/oxygen] - rand(20, 30)))
			*/ // APHELION EDIT REMOVAL END
			var/gas_to_convert = max(0, min(ionize_air_amount, ion_turf_mix.get_moles(/datum/gas/oxygen) - rand(20, 30))) // APHELION EDIT ADDITION - DOGMOS
			if(gas_to_convert <= 0)
				continue
			var/datum/gas_mixture/oxygen_removed_mix = ion_turf_mix.remove_specific(/datum/gas/oxygen, ionize_air_amount)
			/* // APHELION EDIT REMOVAL START - DOGMOS
			if(oxygen_removed_mix && oxygen_removed_mix.moles[/datum/gas/oxygen] > 0)
				var/ion_amount = oxygen_removed_mix.moles[/datum/gas/oxygen] * 0.25
				ion_turf_mix.assert_gas(/datum/gas/tritium)
				ion_turf_mix.moles[/datum/gas/tritium] += ion_amount
			*/ // APHELION EDIT REMOVAL END
			// APHELION EDIT ADDITION START - DOGMOS
			if(oxygen_removed_mix && oxygen_removed_mix.get_moles(/datum/gas/oxygen) > 0)
				var/ion_amount = oxygen_removed_mix.get_moles(/datum/gas/oxygen) * 0.25
				ion_turf_mix.adjust_moles(/datum/gas/tritium, ion_amount)
				// APHELION EDIT ADDITION END
				total_ion_amount += ion_amount

		var/ionization_amount_ratio = total_ion_amount/ionize_air_amount
		var/criticality_to_add = min(ionization_amount_ratio, 3) * rand()
		if(criticality_to_add > 0)
			criticality_to_add = FLOOR(criticality_to_add, 0.01)
			if(criticality >= 100) //It keeps going.
				if(prob(criticality/500)) //The chance to explode. Yes, it's supposed to be this low.
					deconstruct(FALSE)
				else
					criticality += rand(criticality_to_add*4, criticality_to_add*10)
			else
				criticality += criticality_to_add

		playsound(src, 'modular_nova/modules/RBMK2/sounds/ionization.ogg', 50, TRUE, extrarange = ionize_air_range)
	else
		criticality = max(0, criticality-1)

	if(venting)
		if(vent_reverse_direction)
			turf_air.pump_gas_to(buffer_gases, vent_pressure) //Pump turf gases to buffer.
		else
			if(active)
				buffer_gases.pump_gas_to(turf_air, vent_pressure) //Pump buffer gases to turf. Reduced rate because active.
				if(stored_rod)
					transfer_rod_temperature(turf_air, allow_cooling_limiter = TRUE, multiplier = min(1, 0.5*(vent_pressure/200)))
			else
				buffer_gases.pump_gas_to(turf_air, vent_pressure*2) //Pump buffer gases to turf. Increases rate because inactive.
				if(stored_rod)
					transfer_rod_temperature(turf_air, allow_cooling_limiter = FALSE)
		update_appearance(UPDATE_OVERLAYS)
