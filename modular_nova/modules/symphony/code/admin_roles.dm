...
/proc/symphony_discord_admin_sync_enabled()
	return SSsymphony.enabled && SSsymphony.discord_admin_sync
...
/proc/symphony_apply_discord_admins()
	...
	for(var/datum/admin_rank/admin_rank as anything in GLOB.admin_ranks)
		if(!admin_rank?.name)
			continue
		var/list/holders = symphony_ingame_role_ckeys("admin:[admin_rank.name]")
		...
		for(var/holder_ckey in holders)
			if(GLOB.admin_datums[holder_ckey] || GLOB.deadmins[holder_ckey])
				continue
			LAZYOR(ranks_by_ckey[holder_ckey], admin_rank)
	...
	if(!symphony_discord_admin_sync_enabled())
		return 0
	var/granted = 0
	for(var/holder_ckey, holder_rank in ranks_by_ckey)
		if(GLOB.admin_datums[holder_ckey] || GLOB.deadmins[holder_ckey])
			continue
		new /datum/admins(holder_rank, holder_ckey)
		granted++
...
/datum/world_topic/symphony/admin_ranks/Run(list/input)
	...
	.["ranks"] = ranks
	.["discord_sync"] = symphony_discord_admin_sync_enabled()
	// Legacy mode does not read the SQL admin grants written by the panel.
	.["legacy_admin_system"] = SSsymphony.admin_legacy_system
