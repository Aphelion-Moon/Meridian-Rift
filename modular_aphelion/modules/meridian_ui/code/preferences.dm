/datum/preference/choiced/pda_theme/deserialize(input, datum/preferences/preferences)
	// Translate the renamed default before the parent validates the choices - every other theme kept its
	// upstream display name, and an unrecognized one would otherwise be silently reset to the default.
	if(input == PDA_THEME_NTOS_LEGACY_NAME)
