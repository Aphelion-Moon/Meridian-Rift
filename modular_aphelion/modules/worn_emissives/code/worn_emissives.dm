...
/proc/prepare_bodypart_emissive_overlays(bodypart_overlays)
	var/list/appearances = islist(bodypart_overlays) ? bodypart_overlays : null
	var/list/result
	for(var/index in 1 to (isnull(appearances) ? 1 : length(appearances)))
		var/mutable_appearance/source = isnull(appearances) ? bodypart_overlays : appearances[index]
		// Inset husk blood and other composite effects retain their existing plane boundaries.
		if(!source.icon || PLANE_TO_TRUE(source.plane) != EMISSIVE_PLANE || source.layer >= 0 \
		...
		var/mutable_appearance/group = mutable_appearance(layer = source.layer, appearance_flags = EMISSIVE_APPEARANCE_FLAGS)
		group.plane = source.plane // The source plane already includes its multiz offset.
		group.overlays += mask
		result += group
	return isnull(result) ? bodypart_overlays : result
