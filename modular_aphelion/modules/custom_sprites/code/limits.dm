...
/datum/custom_sprite_editor/proc/run_deferred_work()
	if(closing || (!resources_ready && !(pending_work & CUSTOM_SPRITE_WORK_REBUILD)))
		pending_work = NONE
		return TRUE
	var/datum/custom_sprite_pace/pace = pace()
	if((pending_work & CUSTOM_SPRITE_WORK_REBUILD) && !pace.due())
		return FALSE
	...
	if(work & CUSTOM_SPRITE_WORK_REBUILD)
		pace.start()
		if(rebuild_resources(reuse_body = !(work & CUSTOM_SPRITE_WORK_NEW_BODY)))
